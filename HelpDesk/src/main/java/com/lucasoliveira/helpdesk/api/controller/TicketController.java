package com.lucasoliveira.helpdesk.api.controller;
...
import com.lucasoliveira.helpdesk.api.dto.Summary;
import com.lucasoliveira.helpdesk.api.entity.ChangeStatus;
import com.lucasoliveira.helpdesk.api.entity.Ticket;
import com.lucasoliveira.helpdesk.api.entity.User;
import com.lucasoliveira.helpdesk.api.enums.ProfileEnum;
import com.lucasoliveira.helpdesk.api.enums.StatusEnum;
import com.lucasoliveira.helpdesk.api.response.Response;
import com.lucasoliveira.helpdesk.api.security.jwt.JwtTokenUtil;
import com.lucasoliveira.helpdesk.api.service.TicketService;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> createOrUpade (HttpServletRequest request, @RequestBody Ticket ticket, BindingResult result){
		...
		if (number > 0) {
			...
			if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				if (assigned) {
					...
					ticketService.findByParameters(page, count, title, status, priority);
				}
			}else if(userRequest.getProfile().equals(ProfileEnum.ROLE_CUSTOMER)) {
	...
	private void validateChangeStatus(String id, String status, BindingResult result) {
		if (id == null || id.equals("")) {
			result.addError(new ObjectError("Id", "Id no information"));
			return;
		}
		if (status == null || status.equals("")) {
			result.addError(new ObjectError("Status", "Status no information"));
			return;
		}
